...
/* ── Total Budget Bar ── */
function TotalBudgetBar({budget,spent}){
  const pct=budget>0?(spent/budget)*100:0;
  const rem=budget-spent;
  const gc=pct>90?C.neg:pct>75?C.warn:C.pos;
  return(<div style={{background:"#fff",borderRadius:14,padding:"20px 24px",boxShadow:C.sh,border:`1px solid ${C.border}`}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
      <span style={{fontSize:15,fontWeight:700,color:C.text}}>Inversión total</span>
      <span style={{fontSize:13,fontWeight:700,color:gc,background:gc+"14",padding:"4px 14px",borderRadius:20}}>{pct.toFixed(1)}% consumido</span>
    </div>
    <div style={{background:"#f2f2f7",borderRadius:6,height:8,overflow:"hidden",marginBottom:16}}>
      <div style={{height:"100%",width:`${Math.min(pct,100)}%`,background:gc,borderRadius:6,transition:"width 0.8s"}}/>
    </div>
    <div style={{display:"flex",justifyContent:"space-between"}}>
      <div><div style={{fontSize:10,color:C.tt,fontWeight:500,marginBottom:3}}>Invertido</div><div style={{fontSize:18,fontWeight:700,color:C.text}}>{fmt(spent)}</div></div>
      <div style={{textAlign:"center"}}><div style={{fontSize:10,color:C.tt,fontWeight:500,marginBottom:3}}>Presupuesto</div><div style={{fontSize:18,fontWeight:700,color:C.ts}}>{fmt(budget)}</div></div>
      <div style={{textAlign:"right"}}><div style={{fontSize:10,color:C.tt,fontWeight:500,marginBottom:3}}>Restante</div><div style={{fontSize:18,fontWeight:700,color:rem<0?C.neg:C.pos}}>{fmt(rem)}</div></div>
    </div>
  </div>)
}

/* ── Channel Bar (shows % of total spend) ── */
function ChannelBar({label,spent,totalSpent,color}){
  const pctOfTotal=totalSpent>0?(spent/totalSpent)*100:0;
  return(<div style={{background:"#fff",borderRadius:14,padding:"18px 20px",boxShadow:C.sh,border:`1px solid ${C.border}`}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:8,height:8,borderRadius:"50%",background:color}}/><span style={{fontSize:14,fontWeight:600}}>{label}</span></div>
      <span style={{fontSize:12,fontWeight:600,color:color,background:color+"14",padding:"3px 10px",borderRadius:20}}>{pctOfTotal.toFixed(1)}% del total</span>
    </div>
    <div style={{background:"#f2f2f7",borderRadius:4,height:5,overflow:"hidden",marginBottom:14}}>
      <div style={{height:"100%",width:`${Math.min(pctOfTotal,100)}%`,background:color,borderRadius:4,transition:"width 0.8s"}}/>
    </div>
    <div style={{display:"flex",justifyContent:"space-between"}}>
      <div><div style={{fontSize:10,color:C.tt,fontWeight:500,marginBottom:2}}>Invertido</div><div style={{fontSize:15,fontWeight:700}}>{fmt(spent)}</div></div>
      <div style={{textAlign:"right"}}><div style={{fontSize:10,color:C.tt,fontWeight:500,marginBottom:2}}>% de inversión total</div><div style={{fontSize:15,fontWeight:700,color:color}}>{pctOfTotal.toFixed(1)}%</div></div>
    </div>
  </div>)
}
...
export default function Page(){
  ...
  const loadBudgets=useCallback(async()=>{
    try{
      ...
      if(r.ok){const t=await r.text();const rows=pCSV(t);if(rows.length>=12){const b=MO.map((_,i)=>parseFloat(rows[i]?.presupuesto||rows[i]?.Presupuesto||0)||0);if(b.some(v=>v>0))setMb(b)}}
    }catch(e){}
  },[]);

  const saveMb=useCallback(nb=>{setMb(nb);safeSet("seat-budgets-v3",JSON.stringify(nb));setSaving(true);setTimeout(()=>setSaving(false),1000)},[]);
  ...
  // Filter by segment (fixed: supports - _ and space separators)
  ...
  return(<div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:F}}>
    ...
    <div style={{padding:24,maxWidth:1440,margin:"0 auto"}}>
      ...
      {!ld&&data.length>0&&vw==="overview"&&<>
        ...
        {/* Total budget bar */}
        <div style={{marginBottom:14}}><TotalBudgetBar budget={tB} spent={tot.sp}/></div>
        {/* Channel comparison bars */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:20}}>
          <ChannelBar label="Google Ads" spent={gA.sp} totalSpent={tot.sp} color={C.g}/>
          <ChannelBar label="Meta Ads" spent={mA.sp} totalSpent={tot.sp} color={C.m}/>
        </div>
      ...
      {!ld&&vw==="budget"&&<>
        <div style={{background:"#fff4e5",border:"1px solid #ffd699",borderRadius:10,padding:"12px 16px",marginBottom:20,display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{fontSize:13,color:"#8a6d3b"}}>Para que los cambios se vean para todos, edita los presupuestos en el <a href={`https://docs.google.com/spreadsheets/d/${SID}/edit`} target="_blank" rel="noopener noreferrer" style={{color:C.red,fontWeight:600}}>Google Sheet → pestaña Presupuestos</a>. Se reflejan al recargar.</span>
        </div>
